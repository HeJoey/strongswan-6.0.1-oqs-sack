...
GREEN='\33;32mYELLOW='\331;33mBLUE='\33;34mPURPLE=\33035mCYAN=0330NC=\330# No Color

# 测试配置
TEST_COUNT=50ONNECTION_NAME=net-net"
LOG_FILE=ipsec_test_$(date +%Y%m%d_%H%M%S).log"
RESULTS_FILE=figure1csv"
PLOT_SCRIPT=generate_plot.py
# 网络配置
LOCAL_IP=19216800.31.114EMOTE_IP=1920.16800.310.135TERFACE=$(ip route | grep default | awk[object Object]print $5| head -1)

echo -e "${CYAN}========================================$[object Object]NC}
echo -e${CYAN}  strongSwan IPsec 性能测试脚本$[object Object]NC}echo -e ${CYAN}  握手完成时间 (HCT) 测试${NC}
echo -e ==========================${NC}"
...
setup_ideal_network()[object Object]
    ...
    # 设置理想网络条件（无丢包，低延迟）
    sudo tc qdisc add dev$INTERFACE root netem delay5s loss0  
...
- 远程IP: $REMOTE_IP
- 网络接口: $INTERFACE
- strongSwan版本: $(swanctl --version2/dev/null | head-1| echo "Unknown")
EOF
...
def create_hct_plot(results_file):
   创建握手完成时间图表
    ...
    # 图1: 带误差棒的条形图
    ax1bar([平均握手完成时间'], [mean_hct], yerr=std_hct, 
            capsize=10lor='skyblue, edgecolor=navy', linewidth=2)
    ...
    # 在条形图上添加数值标签
    ax1text(0, mean_hct + std_hct + 1, f{mean_hct:00.2f}±{std_hct:.2f}ms', 
    ...
    # 图2: 时间序列图
    ax2.plot(df_success['test_num'], df_success[hct_ms'], o-color='red, alpha=0.7linewidth=1
    ...
    ax2.fill_between(df_success[test_num], mean_hct - std_hct, mean_hct + std_hct, 
                     alpha=0.2lor=blue', label=f±1σ: {std_hct:.2}ms')
    ax2set_xlabel('测试序号', fontsize=12)
    ax2set_ylabel('握手完成时间 (毫秒)', fontsize=12)
    ax2set_title(握手完成时间变化趋势', fontsize=14, fontweight=bold')
    ax2end()
    ax2.grid(True, alpha=0.3    
    plt.tight_layout()
    
    # 保存图表
    plot_filename = f'ipsec_hct_plot_{pd.Timestamp.now().strftime(%Y%m%d_%H%M%S")}.png  plt.savefig(plot_filename, dpi=300box_inches=tight)
...
if __name__ == __main__":
    if len(sys.argv) !=2
        print("用法: python3generate_plot.py <results_file>")
        sys.exit(1)
...
generate_plots()[object Object]
    ...
    if  -f $RESULTS_FILE" ]; then
        python3T_SCRIPT ULTS_FILE      log$[object Object]GREEN}图表生成完成${NC}  else
...
# 主函数
main() [object Object]    log${CYAN}开始IPsec性能测试$[object Object]NC}   
    # 检查依赖
    check_dependencies
    ...
    log "  - 统计数据: statistics_$(date +%Y%m%d_%H%M%S).txt   log  - 图表文件: ipsec_hct_plot_*.png
...
# 信号处理
trap cleanup EXIT
traplog${RED}测试被中断${NC}; exit1 INT TERM
