...
static int ikeauth(vici_conn_t *conn)
{
	vici_req_t *req;
	vici_res_t *res;
	...
	while (TRUE)
	{
		switch (command_getopt(&arg))
		{
			...
			case EOF:
				break;
			default:
				return command_usage("invalid --ikeauth option");
		}
		break;
	}
	...
	if (!ike)
	{
		return command_usage("--ikeauth requires --ike <name>");
	}
	...
	{
		ret = errno;
		fprintf(stderr, "ikeauth request failed: %s\n", strerror(errno));
		return ret;
	}
	if (format & COMMAND_FORMAT_RAW)
	{
		vici_dump(res, "ikeauth reply", format & COMMAND_FORMAT_PRETTY,
				  stdout);
	}
	else
	{
		...
		{
			printf("IKE_AUTH completed successfully\n");
		}
		else
		{
			fprintf(stderr, "IKE_AUTH failed: %s\n",
...
static void __attribute__ ((constructor))reg()
{
	command_register((command_t) {
		ikeauth, 'U', "ikeauth", "initiate IKE_AUTH phase only",
		{"--ike <name> [--timeout <s>] [--raw|--pretty]"},
		{
			{"help",		'h', 0, "show usage information"},
			{"ike",			'U', 1, "initiate IKE_AUTH for IKE_SA"},
