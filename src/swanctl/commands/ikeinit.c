...
static int ikeinit(vici_conn_t *conn)
{
	vici_req_t *req;
	vici_res_t *res;
	...
	while (TRUE)
	{
		switch (command_getopt(&arg))
		{
			...
			case EOF:
				break;
			default:
				return command_usage("invalid --ikeinit option");
		}
		break;
	}
	...
	if (!ike)
	{
		return command_usage("--ikeinit requires --ike <name>");
	}
	...
	{
		ret = errno;
		fprintf(stderr, "ikeinit request failed: %s\n", strerror(errno));
		return ret;
	}
	if (format & COMMAND_FORMAT_RAW)
	{
		vici_dump(res, "ikeinit reply", format & COMMAND_FORMAT_PRETTY,
				  stdout);
	}
	else
	{
		...
		{
			printf("IKE_SA_INIT completed successfully\n");
		}
		else
		{
			fprintf(stderr, "IKE_SA_INIT failed: %s\n",
...
static void __attribute__ ((constructor))reg()
{
	command_register((command_t) {
		ikeinit, 'M', "ikeinit", "initiate IKE_SA_INIT phase only",
		{"--ike <name> [--timeout <s>] [--raw|--pretty]"},
		{
			{"help",		'h', 0, "show usage information"},
			{"ike",			'I', 1, "initiate IKE_SA_INIT for IKE_SA"},
