...
CALLBACK(ikeinit, vici_message_t*,
	private_vici_control_t *this, char *name, u_int id, vici_message_t *request)
{
	peer_cfg_t *peer_cfg = NULL;
	char *ike;
	...
	if (!ike)
	{
		return send_reply(this, "missing IKE configuration name");
	}
	if (timeout >= 0)
	{
		log_cb = (controller_cb_t)log_vici;
	...
	DBG1(DBG_CFG, "vici ikeinit IKE_SA '%s'", ike);
	
	/* Find the peer configuration */
	find_child_cfg(NULL, ike, &peer_cfg);
	if (!peer_cfg)
	{
		return send_reply(this, "IKE_SA config '%s' not found", ike);
	}
	
	/* For now, use a simple message - we'll implement full separation later */
	DBG1(DBG_CFG, "ikeinit: attempting IKE_SA_INIT phase only");
	return send_reply(this, "IKE_SA_INIT phase separation not yet fully implemented");
}

CALLBACK(ikeinter, vici_message_t*,
	private_vici_control_t *this, char *name, u_int id, vici_message_t *request)
{
	peer_cfg_t *peer_cfg = NULL;
	char *ike;
	...
	DBG1(DBG_CFG, "vici ikeinter IKE_SA '%s'", ike);
	...
	/* IKE_INTERMEDIATE is typically handled automatically by the state machine */
	DBG1(DBG_CFG, "ikeinter: IKE_INTERMEDIATE phase is handled automatically");
	return send_reply(this, "IKE_INTERMEDIATE is handled automatically after IKE_SA_INIT");
}

CALLBACK(ikeauth, vici_message_t*,
	private_vici_control_t *this, char *name, u_int id, vici_message_t *request)
{
	peer_cfg_t *peer_cfg = NULL;
	char *ike;
	...
	DBG1(DBG_CFG, "vici ikeauth IKE_SA '%s'", ike);
	...
	/* For now, use a simple message - we'll implement full separation later */
	DBG1(DBG_CFG, "ikeauth: attempting IKE_AUTH phase only");
	return send_reply(this, "IKE_AUTH phase separation not yet fully implemented");
}
...
static void manage_commands(private_vici_control_t *this, bool reg)
{
	...
	manage_command(this, "ikeinit", ikeinit, reg);
	manage_command(this, "ikeinter", ikeinter, reg);
	manage_command(this, "ikeauth", ikeauth, reg);
