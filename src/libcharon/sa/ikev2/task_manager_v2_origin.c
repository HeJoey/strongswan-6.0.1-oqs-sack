 ...
 */
 ...
 METHOD(task_manager_t, set_phase_separation_mode, void,
	 private_task_manager_t *this, bool init_only, bool auth_only)
 {
	 if (init_only && auth_only)
	 {
		 this->phase_separation_mode = 0; // 如果两个都是true，设为正常模式
	 }
	 else if (init_only)
	 {
		 this->phase_separation_mode = 1; // 仅初始化模式
	 }
	 else if (auth_only)
	 {
		 this->phase_separation_mode = 2; // 仅认证模式
	 }
	 else
	 {
		 this->phase_separation_mode = 0; // 正常模式
	 }
	 
	 DBG1(DBG_IKE, "set phase separation mode: init_only=%s, auth_only=%s, mode=%d (0=normal, 1=init_only, 2=auth_only)", 
		   init_only ? "true" : "false", auth_only ? "true" : "false", this->phase_separation_mode);
 }
										...
										uint32_t message_id, uint16_t fragment_number)
 {
	 ...
	 {
		 DBG0(DBG_IKE, "RRR4_FRAGMENT_ACK_FAILED: message_id=%d, fragment_number=%d, "
