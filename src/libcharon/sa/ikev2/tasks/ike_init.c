...
static bool build_payloads(private_ike_init_t *this, message_t *message)
{
	...
	/* notify the peer if we support selective fragment retransmission */
	if (!this->old_sa && 
		lib->settings->get_bool(lib->settings, "%s.selective_fragment_retransmission", 
								TRUE, lib->ns))
	{
			...
			case PLV2_NOTIFY:
			{
				...
				switch (notify->get_notify_type(notify))
				{
					...
					case INTERMEDIATE_EXCHANGE_SUPPORTED:
						...
						break;
					case SELECTIVE_FRAGMENT_RETRANSMISSION_SUPPORTED:
						if (!this->old_sa)
						{
							this->ike_sa->enable_extension(this->ike_sa,
														   EXT_SELECTIVE_FRAGMENT_RETRANSMISSION);
...
// 		 this->ike_sa->get_other_host(this->ike_sa));
	...
	if (!this->ke)
	{
		...
		else
		{
			this->ke_method = ike_cfg->get_algorithm(ike_cfg,
													 KEY_EXCHANGE_METHOD);
			DBG0(DBG_IKE, "ke_method: %N", key_exchange_method_names, this->ke_method);
		}
		this->ke = this->keymat->keymat.create_ke(&this->keymat->keymat,
												  this->ke_method);
		if (!this->ke)
	...
	if (this->my_nonce.ptr == NULL)
	{
		...
		DBG0(DBG_IKE, "my_nonce: %#B", &this->my_nonce);
	}
