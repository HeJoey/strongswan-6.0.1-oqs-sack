...
typedef struct {
	...
	/**
	 * Time when the first fragment was received (for timeout detection)
	 */
	time_t first_received_time;

	/**
	 * Fragment timeout in seconds (default: 60 seconds)
	 */
	...
	enumerator_t **fragments)
{
	...
	for (num = 1; num <= count; num++)
	{
		...
		DBG1(DBG_ENC, "generating fragment %d/%d: data.len=%zu, fragment_len=%zu", 
			 num, count, data.len, len);
		
		if (len == 0)
		{
			DBG1(DBG_ENC, "fragment %d has zero length, skipping remaining fragments", num);
			break;
		}
		...
		if (status != SUCCESS)
		{
			DBG1(DBG_ENC, "failed to generate IKE fragment %d", num);
			clear_fragments(this);
			DESTROY_IF(generator);
			return FAILED;
		}
		DBG1(DBG_ENC, "successfully generated fragment %d", num);
	...
	private_message_t *this, message_t *message)
{
	...
	if (!this->fragments || total > this->frag->last)
	{
		...
		/* Set the time when first fragment is received */
		if (this->frag->first_received_time == 0)
		{
			this->frag->first_received_time = time(NULL);
		}
	}
...
METHOD(message_t, is_fragment_timeout, bool,
	private_message_t *this)
{
	time_t current_time;

	/* Only applicable to defragmentation messages */
	if (!this->frag)
	{
		return FALSE;
	}

	/* If no fragments received yet, no timeout */
	if (this->frag->first_received_time == 0)
	{
		return FALSE;
	}

	/* Check if we have all fragments already */
	if (this->frag->last > 0 && array_count(this->fragments) == this->frag->last)
	{
		return FALSE;
	}
	...
	return (current_time - this->frag->first_received_time) > this->frag->fragment_timeout;
}

METHOD(message_t, get_missing_fragments, uint16_t*,
	private_message_t *this, uint16_t *count)
{
	uint16_t *missing_fragments = NULL;
	...
	*count = 0;
	...
	/* Only applicable to defragmentation messages */
	if (!this->frag || this->frag->last == 0)
	{
		return NULL;
	}

	/* Create a bitmap of received fragments */
	received = calloc(this->frag->last + 1, sizeof(bool));
	if (!received)
	{
		return NULL;
	}

	/* Mark received fragments */
	for (i = 0; i < array_count(this->fragments); i++)
	{
		array_get(this->fragments, i, &fragment);
		if (fragment->num <= this->frag->last)
		{
			received[fragment->num] = TRUE;
		}
	}

	/* Count missing fragments (fragments are numbered 1 to last) */
	for (i = 1; i <= this->frag->last; i++)
	{
		if (!received[i])
		{
			missing_count++;
	...
	if (missing_count > 0)
	{
		/* Allocate array for missing fragment numbers */
		missing_fragments = malloc(missing_count * sizeof(uint16_t));
...
METHOD(message_t, get_received_fragments, uint16_t*,
	private_message_t *this, uint16_t *count)
{
	uint16_t *received_fragments = NULL;
	...
	*count = 0;
	...
	if (received_count > 0)
	{
		...
		received_fragments = malloc(received_count * sizeof(uint16_t));
...
METHOD(message_t, get_total_fragments, uint16_t,
	private_message_t *this)
{
	/* Only applicable to defragmentation messages */
	if (!this->frag)
...
message_t *message_create_from_packet(packet_t *packet)
{
	...
	INIT(this,
		.public = {
			...
			.is_fragment_timeout = _is_fragment_timeout,
			.get_missing_fragments = _get_missing_fragments,
			.get_received_fragments = _get_received_fragments,
...
message_t *message_create_defrag(message_t *fragment)
{
	...
	INIT(this->frag,
		...
		.fragment_timeout = lib->settings->get_int(lib->settings,
								"%s.fragment_timeout", 60, lib->ns),
