...
struct message_t {
	...
	/**
	 * Check if any fragments have timed out during reassembly.
	 *
	 * This method checks if there are missing fragments that have not been
	 * received within the configured timeout period. Only applicable to
	 * messages created with message_create_defrag().
	 *
	 * @return			TRUE if fragment timeout has occurred
	 */
	bool (*is_fragment_timeout)(message_t *this);

	/**
	 * Get a list of missing fragment numbers for selective retransmission.
	 *
	 * This method returns an array of fragment numbers that are still missing
	 * for message reassembly. The caller is responsible for freeing the
	 * returned array.
	 *
	 * @param count		receives the number of missing fragments
	 * @return			array of missing fragment numbers, NULL if none missing
	 */
	uint16_t* (*get_missing_fragments)(message_t *this, uint16_t *count);

	/**
	 * Get a list of received fragment numbers for acknowledgment.
	 *
	 * This method returns an array of fragment numbers that have been received
	 * during message reassembly. The caller is responsible for freeing the
	 * returned array.
	 *
	 * @param count		receives the number of received fragments
	 * @return			array of received fragment numbers, NULL if none received
	 */
	uint16_t* (*get_received_fragments)(message_t *this, uint16_t *count);

	/**
	 * Get the total number of fragments expected for this message.
	 *
	 * This method returns the total number of fragments that are expected
	 * for complete message reassembly.
	 *
	 * @return			total number of fragments expected
	 */
